...
#define PTY_DETAILS_PERMISSION_LIST(a) \
	...
	namespace details {
		...
		template<class T>
			using get_permissions_or_new_tuple = typename get_permissions_or_new_tuple_t<T>::value;
	}

	template<class Base>
		struct ReadOnly : Base {
			typedef meta::prepend<permissions::Read, meta::remove<permissions::Write, details::get_permissions_or_new_tuple<Base>>> Permissions;
		};

	template<class Base>
		struct WriteOnly : Base {
			typedef meta::prepend<permissions::Write, meta::remove<permissions::Read, details::get_permissions_or_new_tuple<Base>>> Permissions;
		};

	template<class Base>
		struct ReadWrite : Base {
			typedef meta::prepend<permissions::Write, meta::prepend<permissions::Read, details::get_permissions_or_new_tuple<Base>>> Permissions;
		};

	template<class Base>
		struct NoAccess : Base {
			typedef meta::remove<permissions::Write, meta::remove<permissions::Read, details::get_permissions_or_new_tuple<Base>>> Permissions;
		};

	namespace permissions {
		template<class P>
		struct is_readable_t {
			constexpr static const bool value =  meta::contains<Read, typename P::Permissions>;
		};

		template<class P>
			constexpr static const bool is_readable = is_readable_t<P>::value;

		template<class P>
		struct is_writable_t {
			constexpr static const bool value =  meta::contains<Write, typename P:: Permissions>;
		};

		template<class P>
			constexpr static const bool is_writable = is_writable_t<P>::value;
